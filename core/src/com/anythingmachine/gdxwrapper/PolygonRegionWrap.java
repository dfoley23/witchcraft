...
public class PolygonRegionWrap {
        ...
        /**
         * Creates a PolygonRegion by reading in the vertices and texture
         * coordinates from the external file. TextureRegion can come from an atlas.
         *
         * @param region
         ...
         */
        public PolygonRegionWrap(TextureRegion region, FileHandle file) {
                this.region = region;
        ...
        /**
         * Creates a PolygonRegin by triangulating the polygon coordinates in
         * vertices and calculates uvs based on that. TextureRegion can come from an
         * atlas.
         *
         * @param region
         * the region used for drawing
         * @param vertices
         * contains 2D polygon coordinates in pixels relative to source
         * region
         */
        public PolygonRegionWrap(TextureRegion region, float[] vertices) {
                this.region = region;

                EarClipTriangulator ect = new EarClipTriangulator();
                ...
                List<Vector2> triangulatedVectors = ect
                                .computeTriangles(polygonVectors);
                ...
                for (int i = 0; i < triangulatedVectors.size(); i++) {
                        ...
                        texCoords[i * 2] = region.getU()
                                        + (localVertices[i * 2] - region.getRegionX())
                                        / region.getRegionWidth();
                        texCoords[i * 2 + 1] = region.getV()
                                        + (1 - (localVertices[i * 2 + 1] - region.getRegionY())
                                                        / region.getRegionHeight());
                }
        }

        /**
         * Loads the vertices and texture data from an external file. The file
         * should look something like this:
         *
         * ------------ // Triangulated vertices data (x, y) in pixel coordinates
         * with origin bottom-left, y-up v 230.0, 230.0, ... // UVs with origin
         * top-left u 0.23, 0.123, ... -------------
         *
        ...
        private void loadPolygonDefinition(FileHandle file) {
                ...
                try {
                        while (true) {
                                ...
                                if (line == null)
                                        ...
                                        for (int i = 0; i < texCoords.length; i += 2) {
                                                ...
                                                localTexCoords[i + 1] = Float
                                                                .parseFloat(texCoords[i + 1]);
                                        }
                        ...
                        throw new GdxRuntimeException("Error reading polygon shape file: "
                                        + file);
                } finally {
                        try {
                                reader.close();
         ...
         * @return the texture coordinates relative to the Texture (atlas) the
         * region is from
         */
        private float[] calculateAtlasTexCoords(float[] localTexCoords) {

                float uvWidth = this.region.getU2() - this.region.getU();
                float uvHeight = this.region.getV2() - this.region.getV2();

                for (int i = 0; i < localTexCoords.length; i += 2) {
                        localTexCoords[i] = this.region.getU() + localTexCoords[i] * uvWidth;
                        localTexCoords[i + 1] = this.region.getV() + localTexCoords[i + 1]
                                        * uvHeight;
                }
