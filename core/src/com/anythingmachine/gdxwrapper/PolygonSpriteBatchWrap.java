...
/**
 * <p>
 * A PolyongSpriteBatch is used to draw 2D polygons that reference a texture
 ...
 * 
 * <p>
 * All drawing commands of the PolyongSpriteBatch operate in screen coordinates.
 ...
 * 
 * <p>
 * A PolyongSpriteBatch is managed. In case the OpenGL context is lost all
 * OpenGL resources a PolyongSpriteBatch uses internally get invalidated. A
 * context is lost when a user switches to another application or receives an
 * incoming call on Android. A SpritPolyongSpriteBatcheBatch will be
 * automatically reloaded after the OpenGL context is restored.
 * </p>
 * 
 * <p>
 * A PolyongSpriteBatch is a pretty heavy object so you should only ever have
 * one in your program.
 * </p>
 * 
 * <p>
 * A PolyongSpriteBatch works with OpenGL ES 1.x and 2.0. In the case of a 2.0
 ...
 */
public class PolygonSpriteBatchWrap {
	private Mesh mesh;
	 ...
	 */
	public PolygonSpriteBatchWrap() {
		this(4000);
	}

	/**
	 * Constructs a PolygonSpriteBatch with the specified size in vertices and
	 * (if GL2) the default shader. See
	 * {@link #PolygonSpriteBatch(int, ShaderProgram)}.
	 */
	public PolygonSpriteBatchWrap(int size) {
		this(size, null);
	}
	 ...
	 * 
	 * @param size
	 *            the batch size in number of vertices(!)
	 * @param defaultShader
	 *            the default shader to use. This is not owned by the
	 *            SpriteBatch and must be disposed separately.
	 */
	public PolygonSpriteBatchWrap(int size, ShaderProgram defaultShader) {
		this(size, 1, defaultShader);
	}

	/**
	 * Constructs a PolygonSpriteBatch with the specified size and number of
	 * buffers and (if GL2) the default shader. See
	 * {@link #PolygonSpriteBatch(int, int, ShaderProgram)}.
	 */
	public PolygonSpriteBatchWrap(int size, int buffers) {
	 ...
	 *            SpriteBatch and must be disposed separately.
	 */
	public PolygonSpriteBatchWrap(int size, int buffers, ShaderProgram defaultShader) {
		...
		vertices = new float[size * Util.VERTEX_SIZE];

		mesh = this.buffers[0];

		if (true && defaultShader == null) {
	...
	/**
	 * @return the rendering color of this PolygonSpriteBatch. Manipulating the
	...
	/**
	 * Draws a polygon region with the bottom left corner at x,y having the
	 * width and height of the region.
	 */
	public void draw(PolygonRegionWrap region, float x, float y) {
		draw(region, x, y, Math.abs(region.getRegion().getRegionWidth()),
				Math.abs(region.getRegion().getRegionHeight()));
	}
	 ...
	 */
	public void draw(PolygonRegionWrap region, float x, float y, float width, float height) {
		if (!drawing)
	...
	/**
	 * Draws the polygon region with the bottom left corner at x,y and
	 * stretching the region to cover the given width and height. The polygon
	 * region is offset by originX, originY relative to the origin. Scale
	 * specifies the scaling factor by which the polygon region should be scaled
	 * around originX, originY. Rotation specifies the angle of counter
	 * clockwise rotation of the rectangle around originX, originY.
	 */
	public void draw(PolygonRegionWrap region, float x, float y, float originX, float originY, float width,
	...
	/**
	 * Draws the polygon region using the given vertices. Each vertices must be
	 * made up of 5 elements in this order: x, y, color, u, v.
	 */
	public void draw(PolygonRegionWrap region, float[] spriteVertices, int offset, int length) {
		if (!drawing)
			throw new IllegalStateException("PolygonSpriteBatch.begin must be called before draw.");
		...
		if (length <= vertices.length) {
			...
			// ay captain, need to split it across multiple batches. Who sends
			// humangous streams like this?!
		}
	}
	...
	private void renderMesh() {
		...
		renderCalls++;
		totalRenderCalls++;
		int verticesInBatch = idx / Util.VERTEX_SIZE;
		...
		if (customShader != null)
			mesh.render(customShader, GL20.GL_TRIANGLES, 0, verticesInBatch);
		else
			mesh.render(shader, GL20.GL_TRIANGLES, 0, verticesInBatch);
