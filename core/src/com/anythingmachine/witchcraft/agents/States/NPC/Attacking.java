...
public class Attacking extends NPCState {
	...
	public void update(float dt) {
		...
		if (Math.abs(GamePlayManager.player.getX() - sm.phyState.body.getX()) > 175) {
			if (GamePlayManager.currentlevel <= sm.me.level
					&& Math.abs(GamePlayManager.player.getY() - sm.phyState.body.getY()) < 4) {
	...
	public void hitStairs(Stairs stairs) {
		...
		if ((ypos < stairs.getHeight() + 4 && stairs.slantRight() == !sm.facingleft
	...
	public boolean transistionOut() {
		if ( false && GamePlayManager.world.isLocked() ) {
