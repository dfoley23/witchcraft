...
import com.anythingmachine.agents.npcs.NPCStaticAnimation;
import com.anythingmachine.agents.npcs.NPCType;
import com.anythingmachine.agents.npcs.NonPlayer;
import com.anythingmachine.agents.player.Player;
...
public class GamePlayManager extends Screen {
	...
	public static Vector2 exitDoorPos;
	...
	private Random rand;
	public static Platform lowestPlatInLevel;
...
//		currentlevel = 3;
		...
		WitchCraft.assetManager.load("data/world/level1/level" + (currentlevel + 1) + ".tmx", TiledMap.class);
		...
		tiledMapHelper.loadCollisions("data/collisions.txt", world, Util.PIXELS_PER_METER, currentlevel + 1);
		...
		npcs.add(new NonPlayer("knight1", new Vector2(354.0f, 100.0f), new Vector2(0.6f, 0.7f),
				"data/npcdata/knights/fredknight", NPCType.KNIGHT));
		npcs.add(new NonPlayer("knight1", new Vector2(800.0f, 100.0f), new Vector2(0.6f, 0.7f),
				"data/npcdata/knights/joeknight", NPCType.KNIGHT));
		npcs.add(new NonPlayer("archer", new Vector2(3000.0f, 100.0f), new Vector2(0.6f, 0.7f),
				"data/npcdata/other/pimlyarcher", NPCType.ARCHER));
		npcs.add(new NonPlayer("archer", new Vector2(300.0f, 100.0f), new Vector2(0.6f, 0.7f),
	...
	public void update(float dt) {
		...
		if (xGrid < WitchCraft.screenWidth * (WitchCraft.scale)) {
			xGrid = Camera.camera.position.x = WitchCraft.screenWidth * (WitchCraft.scale);
		}
		if (yGrid < WitchCraft.screenHeight * (WitchCraft.scale)) {
			Camera.camera.position.y = WitchCraft.screenHeight * (WitchCraft.scale);
		}
	...
	public void draw(Batch batch) {
		if (!initworld) {
			...
			batch.end();
			if (Util.DEV_MODE)
				debugRenderer.render(world, Camera.camera.combined.scale(Util.PIXELS_PER_METER, Util.PIXELS_PER_METER,
...
//		float val = Math.min(hour > 12 ? 1 - ((float) (hour - 12) / 8f) : ((float) (hour - 4) / 8f), 0.9f);
...
//		// System.out.println(hour+" : "+val);
	...
	public static void switchLevel(int l) {
		...
		WitchCraft.assetManager.setLoader(TiledMap.class, new TmxMapLoader(new InternalFileHandleResolver()));
		WitchCraft.assetManager.unload("data/world/level1/level" + (oldlevel + 1) + ".tmx");
		WitchCraft.assetManager.load("data/world/level1/level" + (l) + ".tmx", TiledMap.class);
		WitchCraft.assetManager.finishLoading();

		String levelstr = "level" + l;
		tiledMapHelper.destroyMap();
		tiledMapHelper.loadMap(levelstr);
		tiledMapHelper.loadCollisions("data/collisions.txt", world, Util.PIXELS_PER_METER, l);
		...
		if (exitDoorPos == null) {
			if (l < currentlevel + 1) {
				playerPos.x = levels.get(l - 1) - 48;
			} else {
				playerPos.x = 64;
			}
		} else {
			playerPos = exitDoorPos;
		}
		player.switchLevel(playerPos);
		for (NonPlayer npc : npcs) {
			npc.checkInLevel();
		}
	...
	public static void level1() {
		...
		bgentities.add(new NPCStaticAnimation("treedweller", new Vector3(1772, 80, 0), new Vector2(1.2f, 0.95f),
				"data/npcdata/static/treedweller1", "data/spine/characters"));
		bgentities.add(new NPCStaticAnimation("treedweller", new Vector3(4900, 80, 0), new Vector2(1.2f, 0.95f),
	...
	public static void level2() {
		...
		bgentities.add(new NPCStaticAnimation("treedweller", new Vector3(2772, 80, 0), new Vector2(1.2f, 0.95f),
				"data/npcdata/static/treedweller1", "data/spine/characters"));
		bgentities.add(new NPCStaticAnimation("treedweller", new Vector3(3699, 50, 0), new Vector2(1.2f, 0.95f),
				"data/npcdata/static/treedweller3", "data/spine/characters"));

		entities.add(new NPCStaticAnimation("ox", new Vector3(5696, 212, 0), new Vector2(1.2f, 1.2f),
	...
	public static void level3() {
		...
		entities.add(new NPCStaticAnimation("tiedwitch", new Vector3(3489, 350, 0), new Vector2(0.75f, 0.8f),
				"data/npcdata/static/tiedwitch", "data/spine/characters"));

		entities.add(new NonPlayer("civ_male", new Vector2(4250.0f, 100.0f), new Vector2(0.6f, 0.7f),
				"data/npcdata/civs/billciv", NPCType.CIV));
		entities.add(new NonPlayer("civ_female", new Vector2(5500.0f, 100.0f), new Vector2(0.6f, 0.7f),
				"data/npcdata/civs/saraciv", NPCType.CIV));

		entities.add(new NPCStaticAnimation("shackledmale1", new Vector3(5824, 118, 0), new Vector2(0.75f, 0.8f),
				"data/npcdata/static/shackledfred", "data/spine/characters"));

		bgentities.add(new Particle(new Vector3(4604, 620, 0)));
		bgentities.add(new AnimatedSpringParticle("tiedwitch", new Vector3(4607, 524, 0), new Vector2(0.75f, 0.8f),
				"data/npcdata/static/hungwitch").addRope((Particle) bgentities.get(0), 400, 300, 7f).setMass(20));
		...
		case BEGINNING:
			...
			triggers.add(new CinematicTrigger()
					.addAction(new Lerp(WitchCraft.cam, 0.0f, 0.005f,
							new Vector3(3150 + WitchCraft.screenWidth * 0.5f, 0, 0)))
					.addAction(new FaceLeft(entities.get(1), 0.0f, true))
					.addAction(
							new AnimateTimed("TORCHMOBBING",
									entities.get(
											1),
									3.0f, 6.3f, true))
					.addAction(
					...
					.addAction(
							new ParticleToGamePlay(8.7f,
									new FireEmitter(new Vector3(2770 + WitchCraft.screenWidth * 0.5f, 170, 0), 15, 0.7f,
											4.1f, false, 1.4f, 2.1f, 10, Util.FIRESPEED),
									1))
					.addAction(
							new ParticleToGamePlay(9.7f,
									new FireEmitter(new Vector3(2787 + WitchCraft.screenWidth * 0.5f, 170, 0), 15, 2.2f,
											4.0f, false, 3.6f, 2.6f, 10, Util.FIRESPEED),
									1))
					.addAction(
							new ParticleToGamePlay(10.7f,
									new FireEmitter(new Vector3(2787 + WitchCraft.screenWidth * 0.5f, 257, 0), 100,
											3.7f, 60.0f, false, 6.4f, 1.3f, 10, Util.FIRESPEED),
									1))
					.addAction(new ParticleToGamePlay(10.9f,
							new FireEmitter(new Vector3(2714 + WitchCraft.screenWidth * 0.5f, 205, 0), 20, 1.7f, 60.0f,
									false, 4.4f, 2.3f, 10, Util.FIRESPEED),
							0))
					.addAction(new ParticleToGamePlay(10.9f,
							new FireEmitter(new Vector3(2857 + WitchCraft.screenWidth * 0.5f, 205, 0), 20, 1.7f, 60.0f,
									false, 4.4f, 2.3f, 10, Util.FIRESPEED),
							0))
					// .addAction(
					...
					.addAction(new AnimateTimed("WALKING", entities.get(5), 12.0f, 14.7f, true))
					.addAction(new FaceLeft(entities.get(5), 14.9f, true))
					.addAction(new FaceLeft(entities.get(5), 18.7f, false))
					.addAction(new AnimateTimed("WALKING", entities.get(5), 19.0f, 24.0f, true))
					.addAction(new Lerp(WitchCraft.cam, 22.0f, 0.01f, new Vector3(2500, 0, 0)))
					.buildBody(2850, 1100, 8, 1000));
			// hang witch
			triggers.add(new CinematicTrigger().addAction(new StartParticle(0.0f, bgentities.get(1)))
	...
	public static void level4() {
		...
		entities.add(new TexturedParticle(new Vector3(3100, 160, 0), EntityType.ENTITY,
				((TextureAtlas) WitchCraft.assetManager.get("data/world/otherart.atlas")).createSprite("anvil"),
				new Vector3(0, 0, 0)));

		entities.add(new NonPlayer("civ_male", new Vector2(4250.0f, 100.0f), new Vector2(0.6f, 0.7f),
				"data/npcdata/civs/tempMale4", NPCType.CIV));

		entities.add(new NonPlayer("archer", new Vector2(920.0f, 500.0f), new Vector2(0.6f, 0.7f),
				"data/npcdata/other/tower_archer1", NPCType.ARCHER));
		entities.get(2).faceLeft(true);
		entities.add(new NonPlayer("civ_female", new Vector2(5500.0f, 100.0f), new Vector2(0.6f, 0.7f),
				"data/npcdata/civs/tempFemale4", NPCType.CIV));
		entities.add(new NonPlayer("knight1", new Vector2(354.0f, 100.0f), new Vector2(0.8f, 0.9f),
				"data/npcdata/knights/fredknight", NPCType.KNIGHT));
		entities.add(new NonPlayer("knight1", new Vector2(800.0f, 100.0f), new Vector2(0.7f, 0.7f),
				"data/npcdata/knights/joeknight", NPCType.KNIGHT));
		entities.add(new NonPlayer("archer", new Vector2(3000.0f, 100.0f), new Vector2(0.6f, 0.6f),
				"data/npcdata/other/pimlyarcher", NPCType.ARCHER));
		entities.add(new NonPlayer("archer", new Vector2(300.0f, 100.0f), new Vector2(0.64f, 0.8f),
				"data/npcdata/other/joearcher", NPCType.ARCHER));
		// entities.add(new NonPlayer("civ_male", new Vector2(
		// 4250.0f, 100.0f), new Vector2(0.77f, 0.6f),
		// "data/npcdata/civs/tempMale4", NPCType.CIV));
		entities.add(new NonPlayer("civ_female", new Vector2(6500.0f, 100.0f), new Vector2(0.5f, 0.5f),
				"data/npcdata/civs/tempFemale4", NPCType.CIV));
		WitchCraft.stopMusic();
		// WitchCraft.playMusic("data/sounds/ambient.ogg");
		...
		for (Body b : bodies) {
			if (b.getUserData() instanceof Entity) {
				...
				if (e.type == EntityType.STAIRS) {
					...
					if (posx > 1695 && posx < 1697 && posy > 511 && posy < 513) {
