...
class SynGen{
  ...
  void tones(){
    ...
    for(int cnt = 0; cnt < sampLength; cnt++){
      ...
      double freq = 250.0;//arbitrary frequency
      double sinValue =
        (Math.sin(2*Math.PI*freq*time) +
        Math.sin(2*Math.PI*(freq/1.2)*time) +
        Math.sin(2*Math.PI*(freq/255)*time))/1.4;
      shortBuffer.put((short)(10000*sinValue));
    }//end for loop
  }//end method tones
  //-------------------------------------------//
  ...
  void fmSweep(){
    ...
    double highFreq = 300.0;

    int cnt2 = -3000;
    int cnt3 = -5000;
    int cnt4 = -10000;
    double freq = 0;
    for(int cnt = 0; cnt < sampLength; cnt++,cnt2++,cnt3++,cnt4++){
      double time = cnt/sampleRate;
...
if(cnt > sampLength/1.5){
freq = lowFreq -
             cnt*(highFreq-lowFreq)/sampLength + 100;
	sinValue =  
            Math.sin(2*Math.PI*freq*time);
}//end if
else {
...
//    waWaPulse();
  ...
  void waWaPulse(){
    ...
    for(int cnt1 = 0; cnt1 < sampLength;
      ...
      if(cnt10 > 0){
          val = waWaPulseHelper(
                                  cnt10,sampLength, 200);
        }//end if
      else if(cnt9 > 0){
          val = waWaPulseHelper(
                  cnt10,sampLength, 220);
        }//end if
      else if(cnt8 > 0){
          val = waWaPulseHelper(
                  cnt10,sampLength, 250);
        }//end if
      else if(cnt7 > 0){
          val = waWaPulseHelper(
                  cnt10,sampLength, 250);
  ...
  double waWaPulseHelper(int cnt,int sampLength, float freq){
    //The value of scale controls the rate of
    // decay - large scale, fast decay.
      double scale = 2*cnt;
      if(scale > sampLength) scale = sampLength;
      double gain = 
             8000*(sampLength-scale)/sampLength;
    double time = cnt/sampleRate;
    double sinValue =
      (Math.sin(2*Math.PI*freq*time) +
      Math.sin(2*Math.PI*(freq/1.8)*time) +
