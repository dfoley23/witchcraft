...
import com.anythingmachine.gdxwrapper.ColorWrap;
import com.anythingmachine.gdxwrapper.DataInput;
				...
				for (int ii = 0; ii < itemCount; ii++) {
					...
					case TIMELINE_COLOR: {
						...
						for (int keyframeIndex = 0; keyframeIndex < keyCount; keyframeIndex++) {
							...
							Color.rgba8888ToColor(ColorWrap.tmp, input.readInt());
							timeline.setKeyframe(keyframeIndex, time, ColorWrap.tmp.r, ColorWrap.tmp.g, ColorWrap.tmp.b, ColorWrap.tmp.a);
							if (keyframeIndex < keyCount - 1) readCurve(input, keyframeIndex, timeline);
						}
						timelines.add(timeline);
						duration = Math.max(duration, timeline.getDuration());
