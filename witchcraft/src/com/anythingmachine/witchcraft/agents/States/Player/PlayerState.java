...
public class PlayerState {
	...
	cape.updatePos(
		sm.facingleft ? sm.neck.getWorldX() - 12
	...
	if (sm.hitplatform || sm.hitstairs) {
		...
		// if ( pos.x < sm.elevatedSegment.getUpPosX() ==
		// sm.elevatedSegment.slantRight()
		// && pos.x > sm.elevatedSegment.getDownPosX() ==
		// sm.elevatedSegment.slantRight()) {
		// sm.phyState.body.setX(sm.elevatedSegment
		// .getXPos(groundPoint));
		// }
		if (sm.phyState.body.getY() + 16 > groundPoint) {
		    sm.phyState.body.setY(groundPoint);
		    sm.state.land();
		}
	    } else if (sm.hitplatform
	...
	// || !sm.input.is("down"))
	{
	    ...
	    if (platHeight - 32 < sm.phyState.body.getY() ) {
		    if (!sm.hitstairs || ((sm.elevatedSegment.walkDown() || !sm.input.down()) && (sm.elevatedSegment
			    .slantRight() != sm.facingleft)) || (platHeight - 2 < sm.elevatedSegment
			    .getHeight())) {
		sm.hitstairs = false;
		sm.hitplatform = true;
	...
	float upPos = stairs.getUpPosY();
	boolean slantRight = stairs.slantRight();
	if ((sm.input.up() && (pos.x > stairs.getDownPosX() == sm.facingleft)
		&& sm.facingleft == !slantRight && downPos - 16 < pos.y)
		|| ((upPos - 7 < pos.y) && slantRight == sm.facingleft && (sm.input
			.down() || stairs.walkDown()))
		|| (stairs.isBetween(sm.facingleft, pos.x) && pos.y < upPos
			&& pos.y > downPos && stairs.walkDown())) {
