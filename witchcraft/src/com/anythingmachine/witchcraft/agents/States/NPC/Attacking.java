...
public class Attacking extends NPCState {
	...
	if (Math.abs(GamePlayManager.player.getX() - sm.phyState.body.getX()) > 175) {
	    if (GamePlayManager.currentlevel <= sm.me.level
		    && Math.abs(GamePlayManager.player.getY()
			    - sm.phyState.body.getY()) < 4) {
	...
	if (sm.me.level < GamePlayManager.currentlevel
		&& (sm.phyState.body.getX() + Util.PLAYERRUNSPEED * dt) > GamePlayManager.levels
			.get(sm.me.level)
		...
		&& (sm.phyState.body.getX() + (-Util.PLAYERRUNSPEED * dt) < 64)
		&& sm.me.level > 0) {
	    sm.state.switchLevel(sm.me.level - 1);
	}
    }
	...
	if ((ypos < stairs.getHeight() + 4
		&& stairs.slantRight() == !sm.facingleft && GamePlayManager.player
		.getY() - 2 > ypos)
		|| (ypos > stairs.getHeight() + 4 && stairs.slantRight() == sm.facingleft)
		&& GamePlayManager.player.getY() + 2 < ypos) {
